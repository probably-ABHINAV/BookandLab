          ...
          {(data?.pending_reviews?.length ?? 0) > 0 ? (
            <div className="divide-y divide-[var(--br)]">
              {data.pending_reviews.map((r: any) => {
                const daysWaiting = Math.floor((new Date().getTime() - new Date(r.submitted_at).getTime()) / (1000 * 3600 * 24));
                const isUrgent = daysWaiting > 3;

                return (
                  <div key={r.id} className={`flex flex-col sm:flex-row sm:items-center justify-between p-8 hover:bg-[var(--c2)] transition-colors gap-4 ${isUrgent ? 'bg-red-50 hover:bg-red-100' : ''}`}>
