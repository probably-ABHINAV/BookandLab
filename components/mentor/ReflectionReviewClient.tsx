...
export function ReflectionReviewClient({ submission, student }: ReflectionReviewProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    formData.append("reflectionSubmissionId", submission.id);
    formData.append("chapterId", submission.reflections.chapters.id);
  ...
  if (success) {
    return (
      <div className="bg-blue-50 text-blue-800 p-8 rounded-2xl flex flex-col items-center justify-center space-y-4 border border-blue-200">
        <CheckCircle className="w-12 h-12 text-blue-500" />
        <h2 className="text-xl font-bold">Review Submitted!</h2>
        <p>The student's reflection evaluation has been recorded.</p>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-xl border p-6 shadow-sm">
          <div className="mb-4">
            <span className="text-sm font-semibold text-blue-600 tracking-wider">REFLECTION DEBRIEF</span>
            <h2 className="text-2xl font-bold text-slate-900 mt-1">{submission.reflections.chapters.name}</h2>
          ...
          <div className="prose prose-slate max-w-none">
            <h4 className="text-sm text-slate-400 uppercase tracking-widest font-bold">Reflection Prompt</h4>
            <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100 mt-2 text-indigo-900 font-medium">
              "{submission.reflections.prompt}"
            </div>

            <h4 className="text-sm text-slate-400 uppercase tracking-widest font-bold mt-6">Student's Response</h4>
            ...
            {submission.audio_url && (
              <div className="mt-4">
                <h4 className="text-sm text-slate-400 uppercase tracking-widest font-bold">Voice Recording</h4>
                <div className="mt-2">
      ...
      <div className="bg-white rounded-xl border p-6 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-500">For reflections, you may focus heavily on Critical Thinking and Expression.</p>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Concept Clarity</label>
                <input type="number" name="conceptScore" min="0" max="10" defaultValue="0" required className="block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Critical Thinking</label>
                <input type="number" name="thinkingScore" min="0" max="10" required className="block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Application</label>
                <input type="number" name="applicationScore" min="0" max="10" defaultValue="0" required className="block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Expression</label>
                <input type="number" name="expressionScore" min="0" max="10" required className="block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border" />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea 
              ...
              placeholder="Provide constructive feedback on their reflection..."
            />
          </div>
          ...
          <div>
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer bg-slate-50 border p-3 rounded-lg flex-1 hover:bg-slate-100 transition">
                <input type="radio" name="statusDecision" value="APPROVED" required className="text-blue-600 focus:ring-blue-500" />
                <span className="font-medium text-blue-800">Approve Document</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer bg-slate-50 border p-3 rounded-lg flex-1 hover:bg-slate-100 transition">
